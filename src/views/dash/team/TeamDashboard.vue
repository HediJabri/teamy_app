...
export default {
  ...
  mixins: [utilities],
  components: {
    TeamySpinner, CardUser, CardActivityDash, CardActivityList, CardEventNext, CardTeam, CardTeamEmpty
  },
  data () {
    return {
      todayEvents: [],
  ...
  computed: {
    ...
    teamDataLoaded() {
      if (this.todayEventsLoaded && this.teamActivitiesLoaded) {
        return true
      }
    },
  },
  methods: {
    async getTeamEvents (teamId) {
      try {
        ...
        const teamTodayEvents = data.events.filter(e => this.eventIsToday(e.dateStart))
        const teamNextEvent = data.events.find(e => !this.eventIsToday(e.dateStart))
        if (!this.nextEvent || this.nextEvent && teamNextEvent && teamNextEvent.dateStart <= this.nextEvent.dateStart) {
    ...
    async getTeamActivities (pageNext, teamId) {
      try {
        ...
        pageNext > 0 ?  this.teamActivities = this.teamActivities.concat(data.teamActivities) : this.teamActivities = data.teamActivities
        this.teamActivities.length >= data.teamActivitiesCount ? this.page.allRecordsFetched = true : this.page.allRecordsFetched = false
    ...
    goToNextPage () {
      if (this.page.next >= 0) {
        ...
        this.page.next ++
        if (this.teamActivities) this.getTeamActivities(this.page.next, this.currentTeam._id)
      }
    },
    resetActivities() {
...
@media only screen and (max-width: 479px) {
  .page-right-container { display: none!important; }
  .page-left-container { display: none!important; }
