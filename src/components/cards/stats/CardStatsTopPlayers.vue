...
export default {
  ...
  computed: {
    ...mapGetters([
      'currentUser', 'currentTeam', 'sports', 
      'statsSeasonFilter', 'statsCategoryFilter', 'statsCompetitionFilter', 'statsReloading'
    ]),
  },
  methods: {
    ...
    groupByUser(participations) {
      ...
      for (let participation of participations) {
        const userFinded = topPlayers.find(u => u && u._id === participation.user._id)
        if (userFinded) {
          userFinded.participations.push(participation)
          // Ex: Increment participation.football.goals
          userFinded[this.statsItem.key] += participation[this.currentTeamSportKey()][this.statsItem.key]
        } else {
          let user = Object.assign({}, participation.user,
            { participations: [ participation ] },
            { 'this.statsItem.key': participation[this.currentTeamSportKey()][this.statsItem.key] },
          );
          user[this.statsItem.key] =  participation[this.currentTeamSportKey()][this.statsItem.key]
...
.card-title {
  ...
  .card-title-text {
    @include flex-start();
    h5 { text-transform: uppercase; margin-right: 8px;}
    span { color: $text-grey-blue; font-size: 12px; }
  }
}
.card-body {
  margin-top: 22px;
...
.card-list-item {
  @include list-item-s();
  span { @include tag-flat-s(); min-width: 30px;}
  cursor: initial;
}
