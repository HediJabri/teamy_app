<template>
  <div class="tab-container" v-if="event">
    <div class="tab">
      <div class="tab-header">
        <div class="tab-button">
          <el-button type="primary" v-if="isAdmin(currentUser, event.team)" class="btn-icon"
            ...
            <span v-else> Ajouter un rapport <i class="fa fa-plus-circle margin-left"></i></span>
          </el-button>
        </div>
        <div v-if="!event.reportValidated">
          <p class="tab-title">
            Le rapport te permet d'indiquer les stats chaque joueur 📊 
          </p>
        </div>
      </div>
...
.tab-header {
  @include page-title();
  .tab-button { @include flex-center; width: 100%; margin: 20px 0 30px 0 }
...
@media only screen and (max-width: 759px) {
  .tab { display: none; }
  .tab-message-mobile { @include flex-center; flex-direction: column; }
}
</style>
