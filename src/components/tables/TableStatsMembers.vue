          ...
          <table class="table table-striped">
            <thead>
              <tr>
                ...
                <th v-if="!isTrainingCategory" v-for="(item, index) in currentTeamSport().reportList" :key="index">
                ...
                <th class="column-item-right" :class="{'fix-width': isTrainingCategory}">
                  <div class="column-item">
                    <el-tooltip content="Invitations" placement="top" :open-delay="300">
            ...
            <tbody>
              <table-row-stats-member v-for="membership in validatedMemberships" 
                :key="membership._id" :user="membership.user"/>
            </tbody>
          </table>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...mapGetters(['currentUser', 'currentTeam', 'sports', 'statsEvents', 'statsSeasonFilter', 'statsCategoryFilter', 'statsCompetitionFilter']),
...
.table {
  ...
  .column-item-right.fix-width { width: 50px }
  .column-fix { 
    @include flex-start();
    border: none;
    font-weight: 500;
...
@media only screen and (max-width: 480px) {
  ...
  th.table-item { padding: 15px 5px; }
  .column-item-right:not(.fix-width) { display: none }
}
