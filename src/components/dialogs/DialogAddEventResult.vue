    ...
    <el-dialog title="" :visible.sync="dialogVisible" :fullscreen="smallDevice()" :top="'5vh'">
      <div class="dialog-body">
       <h4 class="dialog-title">
         <span v-if="mode === 'edit'">Modifier le résultat</span>
         <span v-else>Ajouter un résultat</span>
        </h4><br>
        ...
        <div class="dialog-actions">
            ...
            <transition name="fade" mode="out-in">
              <div v-if="!matchEventMode && !result">
                <span class="dialog-result-tag" @click="addResult('win')">
                   <i :class="classResultIcon('win')"></i>Victoire
                </span>
                <span class="dialog-result-tag" @click="addResult('draw')">
                  <i :class="classResultIcon('draw')"></i> Match Nul
                </span>
                <span class="dialog-result-tag" @click="addResult('lost')">
                  <i :class="classResultIcon('lost')"></i> Défaite
                </span>
              ...
              <div v-else-if="matchEventMode && result">
                ...
                <h5 class="dialog-score-title"><strong>Score Final :</strong></h5>
          ...
          <div v-if="!matchEventMode">
            <div class="dialog-actions-switch">
              <el-switch v-model="tournamentMode" active-text="Tournoi" inactive-text="Classement"></el-switch>
            </div>
            <el-select v-model="result" :placeholder="selectPlaceholder">
              <el-option v-for="eventResult in eventResultsList"
                :label="eventResult" :value="eventResult" :key="eventResult">
...
export default {
  ...
  computed: {
    ...
    readyToValidate() {
      if (this.matchEventMode) return this.result && this.scoreTeam != null && this.scoreOpponent != null
      return this.result
    },
  },
  methods: {
    ...
    async validateResult () {
      this.isLoading = true
      const body = { result: this.result, scoreTeam: this.scoreTeam, scoreOpponent: this.scoreOpponent }
      try {
     ...
     async getEvent (id) {
      try {
        ...
        this.$emit('closeDialog')
        this.$notify({ title: 'Succès', message: "Le résultat à bien été ajouté", type: 'success' })
      } catch (err) {
        this.errorNotify(err)
        this.isLoading = false
      }
