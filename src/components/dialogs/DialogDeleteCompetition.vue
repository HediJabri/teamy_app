  ...
  <div v-if="competition">
    <el-dialog title="" :visible.sync="dialogVisible" :fullscreen="smallDevice()">
      <div class="dialog-body">
        ...
        <br>
        <p>Tu es sûr de vouloir supprimer la compétition <strong> {{ competition.name }} </strong> ? 😱</p>
        <p>Toutes les infos de ton équipe relatives à cette compétition seront également supprimées</p>
      </div>
  ...
  methods: {
    async deleteCompetition() {
      ...
      try {
        ...
        this.$notify({ title: 'Succès', message: 'La compétition à bien été suprimée', type: 'success' })
      } catch (err) {
        this.isLoading = false
        this.$emit('closeDialog')
